...
int main() {
	...
	while (!shutdown) {
        ...
        float potValue = analogPot.read();
        uint32_t minRate = 100000;
        uint32_t maxRate = 2000000;
        uint32_t blinkRateUS = minRate + (uint32_t)(potValue * (maxRate - minRate));


        if(timing.tickThreshold(last_task_1_time, TASK_1_RATE_US)){
        	//PROJECT 1 - add code here to actually make the LED blink
        	digitalHeartbeatLED = !digitalHeartbeatLED;
        }
